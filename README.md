...
自作 CPU（FPGA / MCU / ディスクリート CPU 等）を PC に UART 経由で接続し、
PC 上のソフトウェアと接続するための通信プロトコル仕様。CPU 製作者が本書だけを
読めば実装可能なよう、PC 側の実装詳細には踏み込まない。
...
- [6. メッセージ書式](#6-メッセージ書式)
  - [6.1 PC → CPU（CPU への入力）](#61-pc--cpucpu-への入力)
  - [6.2 CPU → PC（CPU からの応答）](#62-cpu--pccpu-からの応答)
...
- [13. 参考](#13-参考)
  ...
  - [13.2 設計判断の背景: なぜ「大文字コマンド」「CR+LF のみ」にしたか](#132-設計判断の背景-なぜ大文字コマンドcrlf-のみにしたか)
...
| 11 | `BS<64char>\r\n` | | BOARD STATUS | CPU が保持している現在の盤面スナップショットを任意タイミングで通知。書式は `BO` と同じ（§7 行優先 64 文字、0=空 / 1=黒 / 2=白）。**報告のみで副作用なし**（PC 側は盤面突合・診断ログ・観戦用に使う）。`BO`（§6.1 #5 PC→CPU、盤面設定）と方向が逆の対。FPGA 最小実装では省略推奨 |
...
- PC 側の端末ソフト（VSCode Serial Monitor / TeraTerm / minicom 等）で手動コマンドを
  投げて挙動確認するのが一般的
- Python pyserial による対局スクリプトの例: 本仕様の各行を手打ち／自動応答させる
  だけで CPU の実装を検証できる

### 13.1 シリアル端末ソフト設定のヒント

§4 の「改行は CR+LF のみ」を反映するため、端末ソフトは CRLF モードで設定する。
...
LF のみで送信すると §4.1 に従い CPU から `ER` が返る。

### 13.2 設計判断の背景: なぜ「大文字コマンド」「CR+LF のみ」にしたか
...
- **行端検出の堅牢性**: `0x0D 0x0A` のシーケンスは ASCII テキスト中で出現しないため、
  ...
  孤立 CR / 孤立 LF はそれだけで §4.1 違反のシグナルとして機能する
- **TX/RX 対称・単一ルール**: CPU 側もホスト側も「`\r\n` で確定、それ以外の改行は
  §4.1 違反」の単一ルールで実装できる。LF のみ規定だと「CR は混入禁止」の
  除外規則を別途明記しないと曖昧になる
...
- **Git autocrlf 問題**: リポジトリにシリアルログや期待出力ファイル (§13 参考) を
